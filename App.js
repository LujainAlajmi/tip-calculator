...
export default function App() {
  ...
  const tipAmount = (value * (tip / 100)) / numOfPeople;
  const total = (value * (1 + tip / 100)) / numOfPeople;
  ...
  return (
    <StyledView className="flex h-full w-full flex-col items-center justify-center bg-[#C5E4E7]">
      <StyledView className=" flex h-[20%] w-full flex-col items-center justify-end pb-5">
      ...
      <StyledView className=" flex h-[90%] w-full flex-col  rounded-t-3xl bg-white p-8">
        <BillInput value={value} setValue={setValue} />
        <SelectTips
          tip={tip}
          setTip={setTip}
          setCustom={setCustom}
